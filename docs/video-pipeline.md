...
- **VENC reader**：独立线程 `GetStream`。用户态队列只保留 1 个 AU，避免预取 4 帧。`SetChnAttr`（含改 FPS）、`RequestIDR`、`close_encoder` 只能在这个线程、两次 `GetStream` 之间做。HTTP 或 `g_mmf_mutex` 上对活通道调这些会和 `GetStream` 抢 `EnterVcodecLock`。VPSS/VENC 延迟 proc 在入队之后读，不要挡 AU。
