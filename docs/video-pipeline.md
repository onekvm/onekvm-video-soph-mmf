...
- **未决热重启问题**：活动视频退出后 MMF 会完成 Stop/Unbind/Reset/Destroy，但下一进程仍可能出现 VPSS 60 FPS、VENC `EncodedFrame=0`。把 Unbind 移到 Stop 前会稳定卡死在厂商 VPU lock 并触发 watchdog，因此不能采用；继续在 MMF 内定位 vendor worker/waitq 的跨进程释放，不得在 Core 或 systemd 加模块重载/清池 workaround。
