...
- **无消费者**：Core `videoLoop` 停在 `waitForConsumer`，不会替 MMF 探 HDMI。分辨率跟随必须在 source 自己的 **HDMI watcher** 里跑。空闲时只用 `CVI_VPSS_DisableChn` 停掉 scaler（`CVI_VIP_SCL`），保留 VPSS→VENC 和 WAVE4 worker；直接解绑会让无输入的 worker 在最终 `StopRecvFrame` 时卡进厂商锁。VI 仍跑，用来判断有没有 HDMI。重新绑定 VENC 或 `GetChnFrame` 时再 `EnableChn`。最终释放先恢复 scaler 并留出一帧时间，再 Unbind producer、StopRecvFrame、Reset/Destroy；不能在 Stop 后仍让 VPSS 投递，否则 stale waitq 会污染下一进程。Enable 失败则保持关闭并让 Core 重试，不要拆 VI。
