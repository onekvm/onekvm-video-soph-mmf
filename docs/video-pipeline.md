...
- **SRTP CryptoDMA**：`write_sample` 在视频线程上同步 `block_on` 加密。`cvitek-spacc` 的 IRQ 等待是 1 s；IRQ 不完成时每一帧 WebRTC 都卡 1 s，UI `actual_fps` 变成 1，而 VENC 仍是 60。第一次 `ETIMEDOUT` 后进程内禁用 offload，后续走软件 AES-GCM。不要在活包路径上重试这个 ioctl。
