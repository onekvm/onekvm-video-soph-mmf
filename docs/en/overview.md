...
`onekvm-video-soph-mmf` installs it. OneKVM loads it at runtime. The CMake
...
That builds `onekvm-video-soph-mmf-runtime` and `onekvm-video-soph-mmf`.
Equivalent shell:

```sh
./scripts/kas.sh shell kas/nanokvm-sd.yml \
  -c 'bitbake -c package_write_ipk onekvm-video-soph-mmf-runtime onekvm-video-soph-mmf'
```

To compile the current working tree without changing recipe `SRCREV`:

```sh
ONEKVM_DEBUG_WORKTREES=onekvm-video-soph-mmf \
  ./scripts/kas.sh shell kas/nanokvm-sd.yml \
  -c 'bitbake -c package_write_ipk onekvm-video-soph-mmf'
```
