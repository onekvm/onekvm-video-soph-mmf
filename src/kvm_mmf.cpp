...
typedef struct {
	...
	VENC_PACK_S capture_packs[MMF_VENC_INTERNAL_PACKS];
...
typedef struct {
	...
	vi_mapping_t vi_mappings[MMF_VI_MAX_CHN][MMF_VI_MAP_CACHE_SIZE];
	...
	VENC_PACK_S enc_jpeg_packs[MMF_VENC_INTERNAL_PACKS];
	uint8_t *enc_jpeg_buffer;
	int enc_jpg_frame_w;
	int enc_jpg_frame_h;
...
static CVI_U32 _mmf_frame_buffer_size(const VIDEO_FRAME_S *frame)
{
...
static CVI_S32 _mmf_free_frame(VIDEO_FRAME_INFO_S *pstVideoFrame)
{
	...
	CVI_U32 mapped_size = _mmf_frame_buffer_size(pstVFrame);
...
static void _mmf_clear_vi_mappings(int ch)
{
	if (ch < 0 || ch >= MMF_VI_MAX_CHN)
		return;
	for (int index = 0; index < MMF_VI_MAP_CACHE_SIZE; ++index) {
		vi_mapping_t *mapping = &priv.vi_mappings[ch][index];
		if (mapping->vir_addr != NULL && mapping->size > 0)
			CVI_SYS_Munmap(mapping->vir_addr, mapping->size);
		memset(mapping, 0, sizeof(*mapping));
	}
	priv.vi_map_next[ch] = 0;
	memset(&priv.vi_frame[ch], 0, sizeof(priv.vi_frame[ch]));
}

static CVI_VOID *_mmf_map_vi_frame(int ch, CVI_U64 phy_addr, CVI_U32 size)
{
	vi_mapping_t *slot = NULL;
	for (int index = 0; index < MMF_VI_MAP_CACHE_SIZE; ++index) {
		vi_mapping_t *mapping = &priv.vi_mappings[ch][index];
	...
	if (slot == NULL) {
		slot = &priv.vi_mappings[ch][priv.vi_map_next[ch] % MMF_VI_MAP_CACHE_SIZE];
		priv.vi_map_next[ch] = (priv.vi_map_next[ch] + 1) % MMF_VI_MAP_CACHE_SIZE;
...
int mmf_vi_frame_pop(int ch, void **data, int *len, int *width, int *height, int *format) {
	...
	if (CVI_VPSS_GetChnFrame(0, ch, frame, 1000) == 0) {
		...
		CVI_VOID *vir_addr = _mmf_map_vi_frame(ch, frame->stVFrame.u64PhyAddr[0], image_size);
		if (vir_addr == NULL) {
			CVI_VPSS_ReleaseChnFrame(0, ch, frame);
			return -1;
		}
...
int mmf_enc_jpg_deinit(int ch)
{
	...
	priv.enc_jpg_running = 0;
	if (priv.enc_jpeg_buffer) {
		free(priv.enc_jpeg_buffer);
		priv.enc_jpeg_buffer = NULL;
	}
...
int mmf_enc_jpg_pop(int ch, uint8_t **data, int *size)
{
	...
	if (stStatus.u32CurPacks > MMF_VENC_INTERNAL_PACKS) {
		printf("JPEG pack count is too large! cnt:%u\r\n", stStatus.u32CurPacks);
		return -1;
	}
	...
	if (priv.enc_jpeg_frame.u32PackCount == 1) {
		VENC_PACK_S *pack = &priv.enc_jpeg_frame.pstPack[0];
	...
	if (priv.enc_jpeg_buffer == NULL) {
		priv.enc_jpeg_buffer = (uint8_t *)malloc(MMF_JPEG_STREAM_BUF_SIZE);
		if (priv.enc_jpeg_buffer == NULL) {
			CVI_VENC_ReleaseStream(ch, &priv.enc_jpeg_frame);
			priv.enc_jpg_running = 0;
			return -1;
	...
	for (CVI_U32 index = 0; index < priv.enc_jpeg_frame.u32PackCount; ++index) {
		...
		if (pack->u32Offset > pack->u32Len ||
			pack->u32Len - pack->u32Offset > MMF_JPEG_STREAM_BUF_SIZE - total) {
			CVI_VENC_ReleaseStream(ch, &priv.enc_jpeg_frame);
			priv.enc_jpg_running = 0;
			return -1;
		}
		CVI_U32 pack_size = pack->u32Len - pack->u32Offset;
		memcpy(priv.enc_jpeg_buffer + total, pack->pu8Addr + pack->u32Offset, pack_size);
		total += pack_size;
	}
	if (data)
		*data = priv.enc_jpeg_buffer;
	if (size)
		*size = total;
...
static int _validate_venc_cfg(int ch, const mmf_venc_cfg_t *cfg)
{
	...
	if (cfg->w <= 0 || cfg->h <= 0 || (cfg->w & 1) || (cfg->h & 1) ||
		cfg->gop <= 0 || cfg->intput_fps <= 0 || cfg->output_fps <= 0 ||
		cfg->output_fps > cfg->intput_fps || cfg->bitrate <= 0) {
		printf("Invalid venc config: %dx%d, gop:%d, fps:%d/%d, bitrate:%d\r\n",
			cfg->w, cfg->h, cfg->gop, cfg->intput_fps, cfg->output_fps, cfg->bitrate);
		return -1;
	}
	return 0;
}

static void _set_venc_common_attr(VENC_CHN_ATTR_S *attr, const mmf_venc_cfg_t *cfg)
{
	memset(attr, 0, sizeof(*attr));
	attr->stVencAttr.enType = cfg->type == 1 ? PT_H265 : PT_H264;
	attr->stVencAttr.u32MaxPicWidth = cfg->w;
	attr->stVencAttr.u32MaxPicHeight = cfg->h;
	attr->stVencAttr.u32BufSize = MMF_VENC_STREAM_BUF_SIZE;
	attr->stVencAttr.bByFrame = CVI_TRUE;
	attr->stVencAttr.u32PicWidth = cfg->w;
	attr->stVencAttr.u32PicHeight = cfg->h;
...
static void _set_h264_cbr_attr(VENC_CHN_ATTR_S *attr, const mmf_venc_cfg_t *cfg)
{
	attr->stRcAttr.enRcMode = VENC_RC_MODE_H264CBR;
	attr->stRcAttr.stH264Cbr.u32Gop = cfg->gop;
	attr->stRcAttr.stH264Cbr.u32StatTime = 2;
	attr->stRcAttr.stH264Cbr.u32SrcFrameRate = cfg->intput_fps;
	attr->stRcAttr.stH264Cbr.fr32DstFrameRate = cfg->output_fps;
	attr->stRcAttr.stH264Cbr.u32BitRate = cfg->bitrate;
	attr->stRcAttr.stH264Cbr.bVariFpsEn = CVI_FALSE;
}

static void _set_h265_cbr_attr(VENC_CHN_ATTR_S *attr, const mmf_venc_cfg_t *cfg)
{
	attr->stRcAttr.enRcMode = VENC_RC_MODE_H265CBR;
	attr->stRcAttr.stH265Cbr.u32Gop = cfg->gop;
	attr->stRcAttr.stH265Cbr.u32StatTime = 2;
	attr->stRcAttr.stH265Cbr.u32SrcFrameRate = cfg->intput_fps;
	attr->stRcAttr.stH265Cbr.fr32DstFrameRate = cfg->output_fps;
	attr->stRcAttr.stH265Cbr.u32BitRate = cfg->bitrate;
	attr->stRcAttr.stH265Cbr.bVariFpsEn = CVI_FALSE;
}

static CVI_S32 _set_venc_rc_param(int ch, uint8_t type)
{
	CVI_S32 ret;
	...
	param.s32FirstFrameStartQp = 35;
	param.s32InitialDelay = 1000;
	if (type == 1) {
		param.stParamH265Cbr.u32MinIprop = 1;
		param.stParamH265Cbr.u32MaxIprop = 10;
		param.stParamH265Cbr.u32MaxQp = 51;
		param.stParamH265Cbr.u32MinQp = 20;
		param.stParamH265Cbr.u32MaxIQp = 51;
		param.stParamH265Cbr.u32MinIQp = 20;
	} else {
		param.stParamH264Cbr.u32MinIprop = 1;
		param.stParamH264Cbr.u32MaxIprop = 10;
		param.stParamH264Cbr.u32MaxQp = 51;
		param.stParamH264Cbr.u32MinQp = 20;
		param.stParamH264Cbr.u32MaxIQp = 51;
		param.stParamH264Cbr.u32MinIQp = 20;
	}
...
static CVI_S32 _set_venc_frame_lost(int ch)
{
	CVI_S32 ret;
	VENC_FRAMELOST_S frame_lost;
	ret = CVI_VENC_GetFrameLostStrategy(ch, &frame_lost);
	if (ret != CVI_SUCCESS) {
		printf("CVI_VENC_GetFrameLostStrategy failed with %d\n", ret);
		return ret;
	}
	frame_lost.enFrmLostMode = FRMLOST_PSKIP;
	ret = CVI_VENC_SetFrameLostStrategy(ch, &frame_lost);
	if (ret != CVI_SUCCESS)
...
{
	if (_validate_venc_cfg(ch, cfg) != 0)
		return -1;

	char name[20];
	snprintf(name, sizeof(name), "venc%.1d", ch);
	uint32_t size = VENC_GetPicBufferSize(cfg->w, cfg->h, (PIXEL_FORMAT_E)cfg->fmt,
	...
	VIDEO_FRAME_INFO_S *capture_frame = (VIDEO_FRAME_INFO_S *)_mmf_alloc_frame(
		pool_id, (SIZE_S){(CVI_U32)cfg->w, (CVI_U32)cfg->h}, (PIXEL_FORMAT_E)cfg->fmt);
	...
	if (cfg->type == 1)
		_set_h265_cbr_attr(&attr, cfg);
	else
		_set_h264_cbr_attr(&attr, cfg);
	...
	if ((ret = _set_venc_rc_param(ch, cfg->type)) != CVI_SUCCESS)
		goto fail_channel;
	if ((ret = _set_venc_frame_lost(ch)) != CVI_SUCCESS)
		goto fail_channel;
	...
	info->capture_stream.pstPack = info->capture_packs;
...
int mmf_del_venc_channel(int ch) {
	...
	if (priv.venc[ch].stream_buffer) {
		free(priv.venc[ch].stream_buffer);
		priv.venc[ch].stream_buffer = NULL;
	}
...
static VIDEO_FRAME_INFO_S *_mmf_find_vi_frame(uint8_t *data, int w, int h, int format)
{
	for (int vi_ch = 0; vi_ch < MMF_VI_MAX_CHN; ++vi_ch) {
		VIDEO_FRAME_INFO_S *frame = &priv.vi_frame[vi_ch];
		if (!priv.vi_chn_is_inited[vi_ch] || frame->stVFrame.pu8VirAddr[0] != data)
			continue;
...
int mmf_venc_push(int ch, uint8_t *data, int w, int h, int format) {
	...
	venc_info_t *info = (venc_info_t *)&priv.venc[ch];
	if (!info->is_inited || w != info->cfg.w || h != info->cfg.h || format != info->cfg.fmt) {
	...
	VIDEO_FRAME_INFO_S *send_frame = _mmf_find_vi_frame(data, w, h, format);
	if (send_frame == NULL) {
		...
		CVI_U32 frame_size = _mmf_frame_buffer_size(&frame_info->stVFrame);
		CVI_SYS_IonFlushCache(frame_info->stVFrame.u64PhyAddr[0],
			frame_info->stVFrame.pu8VirAddr[0], frame_size);
		send_frame = frame_info;
	}

	s32Ret = CVI_VENC_SendFrame(ch, send_frame, 1000);
	if (s32Ret != CVI_SUCCESS) {
...
static int _mmf_export_venc_stream(venc_info_t *info, mmf_stream_t *stream)
{
	if (stream == NULL)
		return 0;

	VENC_STREAM_S *venc_stream = &info->capture_stream;
	if (venc_stream->u32PackCount <= MMF_STREAM_MAX_PACKS) {
		stream->count = venc_stream->u32PackCount;
		for (int index = 0; index < stream->count; ++index) {
			VENC_PACK_S *pack = &venc_stream->pstPack[index];
			if (pack->u32Offset > pack->u32Len)
				return -1;
			stream->data[index] = pack->pu8Addr + pack->u32Offset;
			stream->data_size[index] = pack->u32Len - pack->u32Offset;
		}
		return 0;
	}

	if (info->stream_buffer == NULL) {
		info->stream_buffer = (uint8_t *)malloc(MMF_VENC_STREAM_BUF_SIZE);
		if (info->stream_buffer == NULL)
			return -1;
	}

	CVI_U32 total = 0;
	for (CVI_U32 index = 0; index < venc_stream->u32PackCount; ++index) {
		VENC_PACK_S *pack = &venc_stream->pstPack[index];
		if (pack->u32Offset > pack->u32Len)
			return -1;
		CVI_U32 size = pack->u32Len - pack->u32Offset;
		if (size > MMF_VENC_STREAM_BUF_SIZE - total)
			return -1;
		memcpy(info->stream_buffer + total, pack->pu8Addr + pack->u32Offset, size);
		total += size;
	}
	stream->count = 1;
	stream->data[0] = info->stream_buffer;
	stream->data_size[0] = total;
	return 0;
}
...
int mmf_venc_pop(int ch, mmf_stream_t *stream) {
	...
	venc_stream->pstPack = info->capture_packs;
	...
	if (venc_stream->u32PackCount > MMF_VENC_INTERNAL_PACKS) {
		printf("returned pack count is too large! cnt:%u\r\n", venc_stream->u32PackCount);
		CVI_VENC_ReleaseStream(ch, venc_stream);
		info->is_running = 0;
		return -1;
	}
	if (_mmf_export_venc_stream(info, stream) != 0) {
		printf("failed to export venc stream with %u packs\r\n", venc_stream->u32PackCount);
		CVI_VENC_ReleaseStream(ch, venc_stream);
		info->is_running = 0;
		return -1;
	}
